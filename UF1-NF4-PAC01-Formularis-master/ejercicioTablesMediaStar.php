...
function generate_ratings($rating) {
    $movie_rating = '';
    for ($i = 0; $i < $rating; $i++) {
        $movie_rating .= '<img src="star.png" width="25" heigh="15" alt="star"/>';
    }
    return $movie_rating;
}
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $suma += $row['review_rating']; //Guardamos el valor de cada review_rating
    $contador++; //y contador va sumando.
...
$totales = $suma / $contadoar;

// Bucle for para poder mostrar 'X' numero de estrellas que tenga todo el total
    for ($i=1; $i <$totales; $i++) { 
        $media ='<img src="star.png" width="10" heigh="15px" alt="star"/>';
        echo $media;
    }
...
echo <<<ENDHTML
<form action="http://localhost:8080/UF1-NF4-PAC01-Formularis-master/N4P110form.php?movie_id=$idMovie "method="post">
    <table>
